...
import { DEPOT_POSITION, type RouteStop, type DeliveryRun } from '@/data/sampleData';
import { Badge } from '@/components/ui/badge';
import { Truck, CheckCircle2, Clock, Navigation, MapPin } from 'lucide-react';
...
function LiveMapSVG({ runs, activeRunId }: { runs: DeliveryRun[]; activeRunId: string | null }) {
  ...
  const getColor = (run: DeliveryRun, idx: number) =>
  ...
  return (
    <svg viewBox="0 0 620 380" className="w-full h-full" style={{ background: 'hsl(220 14% 97%)' }}>
      ...
      {[0.2, 0.4, 0.6, 0.8].map(f => (
        <g key={f}>
          <line x1={toSvgX(f * 100)} y1={0} x2={toSvgX(f * 100)} y2={380}
            stroke="hsl(220 14% 90%)" strokeWidth="1" strokeDasharray="4 4" />
          <line x1={0} y1={toSvgY(f * 100)} x2={620} y2={toSvgY(f * 100)}
            stroke="hsl(220 14% 90%)" strokeWidth="1" strokeDasharray="4 4" />
        </g>
      ))}

      {/* Road-like blocks (decorative) */}
      {[
      ...
      {visibleRuns.map((run, idx) => {
        ...
        return (
          <path key={run.id} d={d} fill="none" stroke={color} strokeWidth="2.5"
            strokeDasharray="7 4" opacity={activeRunId && activeRunId !== run.id ? 0.2 : 0.8} />
        );
      })}

      {/* Depot */}
      <rect x={toSvgX(depot.x) - 16} y={toSvgY(depot.y) - 16} width={32} height={32}
        rx={5} fill="hsl(221 83% 53%)" />
      <text x={toSvgX(depot.x)} y={toSvgY(depot.y) + 6} textAnchor="middle"
        fontSize="13" fill="white" fontWeight="bold">W</text>
      <text x={toSvgX(depot.x)} y={toSvgY(depot.y) + 27}
        textAnchor="middle" fontSize="9" fill="hsl(221 83% 53%)" fontWeight="bold">DEPOT</text>
      ...
      {visibleRuns.map((run, idx) => {
        ...
        return run.stops.map(stop => {
          const cx = toSvgX(stop.x);
          const cy = toSvgY(stop.y);
          ...
          return (
            <g key={`${run.id}-${stop.orderId}`} opacity={dimmed ? 0.2 : 1}>
              ...
              <text x={cx} y={cy + 6} textAnchor="middle" fontSize="12" fill="white" fontWeight="bold">
                {stop.stopNumber}
              </text>
              <text x={cx} y={cy + 28} textAnchor="middle" fontSize="8" fill="hsl(220 9% 40%)" fontWeight="500">
      ...
      {runs.map((run, idx) => {
        ...
        return (
          <g key={`leg-${run.id}`}>
            ...
            <text x={26} y={19 + idx * 22} fontSize="9" fill="hsl(220 9% 46%)" fontWeight="500">
              {run.runNumber} · {run.driverName.split(' ')[0]}
            </text>
          </g>
        );
      })}
...
export default function MapPage() {
  const { deliveryRuns, couriers } = useDelivery();
    ...
    <div className="space-y-5">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-0.5">
          {deliveryRuns.length} active run(s) · click a run to highlight its route
        </p>
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-5">
        ...
        <div className="space-y-3">
          ...
          {deliveryRuns.map(run => {
            ...
            return (
              <button
                key={run.id}
                onClick={() => setActiveRunId(isSelected ? null : run.id)}
                className={cn(
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <Badge variant="outline" className={cn('text-[10px] px-1.5 h-4', statusColor(run.status))}>
                    {run.status}
                  </Badge>
                </div>
                ...
                <div>
                  <div className="flex justify-between text-[10px] text-muted-foreground mb-1">
                    ...
                    <span className="flex items-center gap-1">
                      <Clock className="h-2.5 w-2.5" /> {run.estimatedDuration}
                    </span>
                  </div>
                  <div className="h-1.5 rounded-full bg-secondary overflow-hidden">
                    <div
                      className={cn('h-full rounded-full', run.status === 'Completed' ? 'bg-green-500' : 'bg-primary')}
                      style={{ width: `${run.stops.length > 0 ? Math.round((completed / run.stops.length) * 100) : 0}%` }}
                    />
                  </div>
                </div>
        ...
        <div className="lg:col-span-2 space-y-3">
          <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">
            {activeRun ? `Route: ${activeRun.runNumber}` : 'All Active Routes'}
          </p>
          ...
          {/* Selected run stop detail */}
          {activeRun && (
            <div className="rounded-xl border border-border bg-card p-4 space-y-3">
              <div className="flex items-center justify-between">
                <p className="text-sm font-semibold text-foreground">
                  {activeRun.runNumber} — Stop Sequence
                </p>
